...
uint32_t sEE_DOU_ReadBuffer(uint8_t* pBuffer, uint16_t ReadAddr, uint16_t* NumByteToRead)
{
	 uint16_t  page_cnt=0,page_offset,page_readcnt;
  /* Get number of reload cycles */
	page_cnt = ReadAddr % 255+(*NumByteToRead)>255?1:0;
	if(!page_cnt)
	{
	   sEE_ReadBuffer(pBuffer,ReadAddr,NumByteToRead);
	}
  else 
	{
		 page_offset=256-(ReadAddr % 256);
	   sEE_ReadBuffer(pBuffer,ReadAddr,&page_offset);
		 page_offset=256-(ReadAddr % 256);
		 page_readcnt=*NumByteToRead-page_offset;
		 sEE_ReadBuffer(pBuffer+page_offset,ReadAddr+page_offset,&page_readcnt);
	}	
}
/**
