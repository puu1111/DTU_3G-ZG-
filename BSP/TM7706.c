...
//	GPIO_Init(PORT_RESET, &GPIO_InitStructure);
	...
	*/	
	...
	TM7705_SyncSPI(0);		/* 同步SPI接口时序 */
  TM7705_SyncSPI(1);
	TM7705_SyncSPI(2);
	bsp_DelayMS(5);

	/* 配置时钟寄存器 */
	TM7705_WriteByte(REG_CLOCK | WRITE | CH_1,0);			/* 先写通信寄存器，下一步是写时钟寄存器 */
	
	TM7705_WriteByte(CLKDIS_0 | CLK_4_9152M | FS_500HZ,0);	/* 刷新速率50Hz */
	
	TM7705_WriteByte(REG_CLOCK | WRITE | CH_1,1);			/* 先写通信寄存器，下一步是写时钟寄存器 */
	
	TM7705_WriteByte(CLKDIS_0 | CLK_4_9152M | FS_500HZ,1);	/* 刷新速率50Hz */
	
	TM7705_WriteByte(REG_CLOCK | WRITE | CH_1,2);			/* 先写通信寄存器，下一步是写时钟寄存器 */
	
//	TM7705_WriteByte(CLKDIS_0 | CLK_4_9152M | FS_50HZ,2);	/* 刷新速率50Hz */
	TM7705_WriteByte(CLKDIS_0 | CLK_4_9152M | FS_500HZ,2);	/* 刷新速率500Hz */
...
void TM7705_WriteReg(uint8_t _RegID, uint32_t _RegValue,uint8_t num)
{
	...
	TM7705_WriteByte(_RegID | WRITE,num);	/* 写通信寄存器, 指定下一步是写操作，并指定写哪个寄存器 */
...
uint32_t TM7705_ReadReg(uint8_t _RegID,uint8_t num)
{
	...
	TM7705_WriteByte(_RegID | READ,num);	/* 写通信寄存器, 指定下一步是写操作，并指定写哪个寄存器 */
...
void TM7705_CalibSelf(uint8_t _ch,uint8_t num)
{
	...
	{
		/* 自校准CH1 */
		TM7705_WriteByte(REG_SETUP | WRITE | CH_1,num);	/* 写通信寄存器，下一步是写设置寄存器，通道1 */		
		TM7705_WriteByte(MD_CAL_SELF | __CH1_GAIN_BIPOLAR_BUF | FSYNC_0,num);/* 启动自校准 */
		TM7705_WaitDRDY(num);	/* 等待内部操作完成 --- 时间较长，约180ms */
	}
	else if (_ch == 2)
	{
		/* 自校准CH2 */
		TM7705_WriteByte(REG_SETUP | WRITE | CH_2,num);	/* 写通信寄存器，下一步是写设置寄存器，通道2 */
		TM7705_WriteByte(MD_CAL_SELF | __CH2_GAIN_BIPOLAR_BUF | FSYNC_0,num);	/* 启动自校准 */
		TM7705_WaitDRDY(num);	/* 等待内部操作完成  --- 时间较长，约180ms */
	}
	else if (_ch == 3)
	{
		/* 自校准CH2 */
		TM7705_WriteByte(REG_SETUP | WRITE | CH_3,num);	/* 写通信寄存器，下一步是写设置寄存器，通道2 */
		TM7705_WriteByte(MD_CAL_SELF | __CH3_GAIN_BIPOLAR_BUF | FSYNC_0,num);	/* 启动自校准 */
		TM7705_WaitDRDY(num);	/* 等待内部操作完成  --- 时间较长，约180ms */
	}
}
...
void TM7705_SytemCalibZero(uint8_t _ch,uint8_t num)
{
	if (_ch == 1)
	{
		/* 校准CH1 */
		TM7705_WriteByte(REG_SETUP | WRITE | CH_1,num);	/* 写通信寄存器，下一步是写设置寄存器，通道1 */
		TM7705_WriteByte(MD_CAL_ZERO | __CH1_GAIN_BIPOLAR_BUF | FSYNC_0,num);/* 启动自校准 */
		TM7705_WaitDRDY(num);	/* 等待内部操作完成 */
	}
	else if (_ch == 2)
	{
		/* 校准CH2 */
		TM7705_WriteByte(REG_SETUP | WRITE | CH_2,num);	/* 写通信寄存器，下一步是写设置寄存器，通道1 */
		TM7705_WriteByte(MD_CAL_ZERO | __CH2_GAIN_BIPOLAR_BUF | FSYNC_0,num);	/* 启动自校准 */
		TM7705_WaitDRDY(num);	/* 等待内部操作完成 */
	}
		else if (_ch == 2)
	{
		/* 校准CH2 */
		TM7705_WriteByte(REG_SETUP | WRITE | CH_3,num);	/* 写通信寄存器，下一步是写设置寄存器，通道1 */
		TM7705_WriteByte(MD_CAL_ZERO | __CH3_GAIN_BIPOLAR_BUF | FSYNC_0,num);	/* 启动自校准 */
		TM7705_WaitDRDY(num);	/* 等待内部操作完成 */
	}
}
...
void TM7705_SytemCalibFull(uint8_t _ch,uint8_t num)
{
	if (_ch == 1)
	{
		/* 校准CH1 */
		TM7705_WriteByte(REG_SETUP | WRITE | CH_1,num);	/* 写通信寄存器，下一步是写设置寄存器，通道1 */
		TM7705_WriteByte(MD_CAL_FULL | __CH1_GAIN_BIPOLAR_BUF | FSYNC_0,num);/* 启动自校准 */
		TM7705_WaitDRDY(num);	/* 等待内部操作完成 */
	}
	else if (_ch == 2)
	{
		/* 校准CH2 */
		TM7705_WriteByte(REG_SETUP | WRITE | CH_2,num);	/* 写通信寄存器，下一步是写设置寄存器，通道1 */
		TM7705_WriteByte(MD_CAL_FULL | __CH2_GAIN_BIPOLAR_BUF | FSYNC_0,num);	/* 启动自校准 */
		TM7705_WaitDRDY(num);	/* 等待内部操作完成 */
	}
	else if (_ch == 3)
	{
		/* 校准CH2 */
		TM7705_WriteByte(REG_SETUP | WRITE | CH_3,num);	/* 写通信寄存器，下一步是写设置寄存器，通道1 */
		TM7705_WriteByte(MD_CAL_FULL | __CH3_GAIN_BIPOLAR_BUF | FSYNC_0,num);	/* 启动自校准 */
		TM7705_WaitDRDY(num);	/* 等待内部操作完成 */
	}
}
...
uint16_t TM7705_ReadAdc(uint8_t _ch,uint8_t num)
{
	...
	for (i = 0; i < 2; i++)
	{
		TM7705_WaitDRDY(num);		/* 等待DRDY口线为0 */		
